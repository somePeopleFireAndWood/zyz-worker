...
- The sole write exception is acknowledging an exact reconnect challenge you actually observed with `hooks/scripts/agent-runtime-state.sh probe-ack <task-dir> <your-agent-id> <probe-id>`. This is bookkeeping, not a code/test/design change; a heartbeat is never ACK.
