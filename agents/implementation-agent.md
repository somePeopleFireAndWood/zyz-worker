...
- Execute testAgent's mutation manifest (see its `## Mutation Manifest`): apply each listed mutation, run the named cases, report each entry KILLED or SURVIVED, then restore and verify the tree is byte-identical (`git status` clean or diff empty). A coverage claim without a recorded killed mutation is not evidence — this is what turns `Tested: true` from "green" into "green + discriminating".
