...
#
#  - .claude-plugin/plugin.json    : exactly one '"version"' line == "0.5.0"
#  - .claude-plugin/marketplace.json: exactly one '"version"' line == "0.5.0"
#       (no top-level version field; only plugins[0].version)
#  - .codex-plugin/plugin.json     : exactly one '"version"' line matching
#       "0.5.0+codex.<14 digits>"
# ---------------------------------------------------------------------------
run_T2() {
...
$(printf '%s\n' "$hits" | sed 's/^/      | /')"
            ...
            # Exact match against "version": "0.5.0"
            if printf '%s\n' "$hits" | grep -qE '"version"[[:space:]]*:[[:space:]]*"0\.5\.0"'; then
                pass "T2 .claude-plugin/plugin.json has exactly one \"version\": \"0.5.0\" line"
            else
                fail "T2 .claude-plugin/plugin.json '\"version\"' line is not \"0.5.0\".  Line:
$(printf '%s\n' "$hits" | sed 's/^/      | /')"
            fi
        fi
    fi
...
$(printf '%s\n' "$hits" | sed 's/^/      | /')"
            ...
            if printf '%s\n' "$hits" | grep -qE '"version"[[:space:]]*:[[:space:]]*"0\.5\.0"'; then
                pass "T2 .claude-plugin/marketplace.json has exactly one \"version\": \"0.5.0\" line (plugins[0].version, no top-level version)"
            else
                fail "T2 .claude-plugin/marketplace.json '\"version\"' line is not \"0.5.0\".  Line:
$(printf '%s\n' "$hits" | sed 's/^/      | /')"
            fi
        fi
    fi
...
$(printf '%s\n' "$hits" | sed 's/^/      | /')"
            ...
            # Match "0.5.0+codex.<14 digits>"
            if printf '%s\n' "$hits" | grep -qE '"version"[[:space:]]*:[[:space:]]*"0\.5\.0\+codex\.[0-9]{14}"'; then
                pass "T2 .codex-plugin/plugin.json '\"version\"' matches \"0.5.0+codex.<14 digits>\""
            else
                fail "T2 .codex-plugin/plugin.json '\"version\"' does NOT match \"0.5.0+codex.<14 digits>\".  Line:
$(printf '%s\n' "$hits" | sed 's/^/      | /')"
            fi
        fi
    fi
...
run_T4() {
    ...
    if [ ! -x "$pack" ]; then
        ...
        for i in \
            ...
            "dist/zyz-worker-0.5.0.zip exists" \
