...
「写测试的不跑测试，跑测试的不写测试」是刻意的：让测试的作者与执行者分离，避免为了让测试通过而改测试。
...
§3 实现    implementation-agent 与 test-agent **默认并行**（都从设计文档取活）
   ↓       → 跑测试 → 失败分类（实现 bug 归实现、无效测试归测试）→ review → 循环
   ↓       可选拆 SubTask；按**依赖图**而非列表顺序调度
§3.C 汇总  汇总测试（unit/e2e/regression[/pressure]）+ 汇总评审（4 个覆盖维度）
   ↓
§4 交付    核对 Total Goal → 校验注册完整 → 提交推送 → 出最终报告
