# execute-task Skill 设计

## 目标

execute-task Skill 用于帮助用户更好地完成一个任务的端到端执行。

该 Skill 面向已经明确要执行的任务场景。无论任务是独立服务、完整模块，还是少量改动，都必须先完成设计文档，再进入实现、测试、审核和交付。
...
- 所有开发都以设计文档为准。
- 设计文档必须正确、清晰、自洽、完整，使后续 implementationAgent、testAgent、reviewAgent 能完全遵循文档完成实现、测试和审核，尽量无需再次向用户提问。
- 主agent 负责任务推进和调度，不直接写代码、改代码、测试或 review。
- implementationAgent、testAgent、reviewAgent 各自有明确职责边界。
- 任务状态文件贯穿设计、实现、测试、审核和交付全流程，用于记录进度和支持中断恢复。
- implementation 环节尽量不中途提问。遇到非阻塞歧义时，Agent 应选择合理方案并继续；只有会导致数据破坏或无法继续时才停止并向用户提问。
...
其中主agent 不是 subAgent。主agent 是当前与用户对话的 Agent 在执行该 Skill 时采用的主控角色；implementationAgent、testAgent、reviewAgent 才是可被调度或模拟的提示词式 subAgent。

`skills/execute-task/SKILL.md` 必须内置主agent 的核心规则，并明确完整主控提示词位于 `skills/execute-task/prompts/main-agent.md`。这样即使执行环境没有自动读取额外 prompt 文件，主agent 也能遵守基本职责边界；当环境支持读取引用文件时，应优先加载完整主控提示词。
...
- 记录任务状态和整体进度。
- 在设计环节协助用户将本次任务整理为 Markdown 格式的设计文档。
...
- 在 implementation 环节调度 implementationAgent、testAgent、reviewAgent 完成任务实现。
...
implementationAgent 根据设计文档完成工程实现。
...
- 如果当前环境存在适合当前任务的可选能力，例如 llmdoc、superpowers 或其他专项插件，implementationAgent 可以使用它们辅助理解设计、检查实现方案、提升代码质量或优化开发流程。
...
- implementationAgent 完成实现后，负责执行测试代码，包括单元测试、e2e 测试、压力测试、回归测试等。
- implementationAgent 可以运行测试代码，以便直接获得报错信息，减少不必要的信息传递和上下文重复。
- implementationAgent 执行测试时必须注意测试环境，例如本地启动、容器中启动、远程执行等。

限制：

- implementationAgent 不能直接修改测试代码。
- 测试代码只能由 testAgent 编辑。

测试失败处理：

- 如果测试没有通过，implementationAgent 先评估失败原因。
- 如果失败原因是客观存在的工程代码问题，implementationAgent 修复工程代码。
- 如果失败原因是测试内容不合理，implementationAgent 拒绝修改工程代码，并通知 testAgent 修改测试样例或测试内容。
- 拒绝修改时必须说明原因，供 testAgent 或 reviewAgent 后续判断。

Review 意见处理：

- 如果 reviewAgent 认为工程代码应该修改，implementationAgent 先评估审查意见。
- 如果审查意见指出的是客观存在的问题，implementationAgent 修复工程代码。
- 如果审查意见不合理，implementationAgent 拒绝修改，并通知 reviewAgent 拒绝修改的原因。

### testAgent

testAgent 根据设计文档和 implementationAgent 的反馈编写和维护测试代码。
...
- 根据设计文档编写测试代码。
- 根据 implementationAgent 在实现中发现的新测试点，补充或修改测试代码。
- 根据 reviewAgent 的审查意见，补充或修改测试代码。
- 当 implementationAgent 或 reviewAgent 认为测试代码需要修改时，先评估修改意见是否合理。
...
- 在设计环节审查设计文档。
- 在 implementation 环节审查工程内容，包括代码、提示词、静态文件、配置文件等。
- 在 implementation 环节审查测试代码。
- 根据主 agent、implementationAgent、testAgent 对审查意见的修改或拒绝原因，执行新的 review。
...
- 没有冲突。
- 足够完整、完备。
- 后续 implementation 环节无需再向用户提问即可执行。
...
- 主 agent、implementationAgent、testAgent 可能接受审查意见并修改。
- 主 agent、implementationAgent、testAgent 也可能拒绝审查意见，并给出拒绝原因。
- 接受和拒绝可能同时存在。
...
- 自洽。
- 完整。
- 能让后续 implementationAgent、testAgent、reviewAgent 完全遵循设计文档完成实现、测试和审核，尽量无需再向用户提问。
...
12. 用户人工审查通过后，进入 implementation 环节。

### 阶段二：Implementation 环节

implementation 环节由主agent 调度 implementationAgent、testAgent、reviewAgent 完成。

#### 并行开发

- implementationAgent 根据设计文档开发工程代码和相关工程内容。
...
- implementationAgent 与 testAgent 都完成后，implementationAgent 执行测试。
- 测试可包括单元测试、e2e 测试、压力测试、回归测试等。
- implementationAgent 执行测试前应确认测试环境，例如本地、容器或远程。
- implementationAgent 可以重复执行测试，以便直接获得报错信息并修复工程代码。
- implementationAgent 不得修改测试代码。

#### Bug 修复

- 如果测试全部通过，进入 review。
- 如果测试失败，implementationAgent 判断失败原因。
- 如果是工程代码问题，implementationAgent 修复工程代码后重新执行测试。
- 如果是测试代码问题，implementationAgent 通知 testAgent 修改测试。
- testAgent 修改测试后，implementationAgent 重新执行测试。
...
- implementationAgent 和 testAgent 分别评估与自身职责相关的审查意见。
...
- 测试由 implementationAgent 执行。
- 测试全部通过后，reviewAgent 基于最新代码、测试代码和拒绝修改原因执行新的 review。
- 当测试全部通过，且 reviewAgent 判断无需修改时，implementation 环节完成。

#### 长时间任务与异常恢复

implementation 环节可能执行很长时间，且中途可能不稳定。
...
任务状态文件由主agent 维护，贯穿设计环节、implementation 环节和交付环节。
...
- 不得因为缺少可选增强能力而阻塞设计、implementation、测试、review 或交付。
...
- `.claude/agents/implementation-agent.md`、`.claude/agents/test-agent.md`、`.claude/agents/review-agent.md` 是 Claude Code 原生项目级 subAgent 定义。
- 主agent 不放入 `.claude/agents/`，因为主agent 是当前与用户对话的控制角色。

建议演进顺序：

1. 使用 `skills/execute-task/SKILL.md` 和 `subagents/` 中的提示词验证工作流。
