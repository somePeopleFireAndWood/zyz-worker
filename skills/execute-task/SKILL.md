...
**The body is authoritative; the summary is a map.** This section is a second description of the same design, which is exactly the surface that rots (see Design Document Edit Discipline). Three rules keep it honest:
...
- **Repair by REGENERATING the affected part of the summary from the body**, not by patching the one sentence that was caught. A divergence means the summary was derived from a design state that no longer exists, so the neighbouring claims are suspect too — re-derive the whole affected part (the module list, the flow, or the attention entry) by reading the body as it now stands. Never annotate the summary with what changed; never reconcile by softening the body.

implementation-agent and test-agent build from the body, not from the summary. reviewAgent checks summary-vs-body consistency as part of design review and reports a divergence as a blocking finding (it can mislead the approving human, so it reaches the artifact through the approval decision).
