...
- This arms the plugin's watchdog layer (heartbeat hooks, exit gates, background monitor — see `skills/execute-task/SKILL.md` `## Watchdog Enforcement`); without a resolvable pointer the watchdog silently no-ops. **Confirm it armed**: after a few tool calls, check that `<task-dir>/runtime/agents/main.heartbeat` exists (the heartbeat is async, so not on the first call). If it is missing, state plainly that the watchdog is inert instead of assuming protection — an unarmed layer looks exactly like a healthy quiet one.
...
- **When resuming an in-flight task, verify the status file before trusting it** (see `SKILL.md` `## Resuming An Existing Task`): cross-check against `git log`/`git status`, against `subtasks/*.md`, and against `runtime/agents/*.heartbeat`. The heartbeat check is what separates "that role died and its work is lost" from "that role is still running" — re-dispatching a live role puts two agents in one workspace writing the same files. Reconcile and rewrite the main file immediately, marking which parts were reconstructed from code.
