...
- You schedule. You do not execute. Each task is run by a separate Claude Code or Codex process (worker, L3) inside a dedicated tmux session, invoking the execute-task skill.
...
- Optional environment variables:
  ...
  - `ZYZ_WORKER_MCP` (default `none`) — worker MCP inheritance policy, snapshotted into each worker's `dispatch.md` `worker-mcp-args:` at spawn/reuse time by `scripts/orch-worker-mcp-args.sh`:
    - `none` (default): Claude uses `--strict-mcp-config`; Codex snapshots every currently-enabled server from `codex mcp list --json` into explicit `-c mcp_servers.<name>.enabled=false` overrides (interactive Codex does not accept the exec-only `--ignore-user-config`). Workers get **zero** MCP servers. Set `inherit` to restore inheritance.
    - `inherit`: legacy behavior; no flag appended, workers inherit the host's global `~/.claude.json` `mcpServers` in full.
    - `<config-path>`: Claude launches with `--strict-mcp-config --mcp-config '<path>'`. Codex has no equivalent interactive single-file override, so it warns and fails closed to `none`. An invalid path also fails closed. ⚠️ See the README's shared-MCP security note.
...
For each task selected, dispatch happens in two stages: **spawn the container** (build one worktree per repo + a single tmux session + heartbeat), then **dispatch an L2 driver** to start the selected agent runtime + execute-task. Spawn never starts the agent; only the L2 driver does.
...
- On exit code 0:
  ...
  - Read the worker's `dispatch.md` for `tmux-pane-id`, `shell-pid`, `worktree`, the numbered worktree set, `plugin-root`, `agent-runtime`, and `worker-runtime-args` (fall back to legacy `worker-mcp-args`).
  - **Dispatch an L2 `orch-driver-agent` with `intent=first-dispatch`**. Its self-contained prompt MUST include `task-id`, `list-dir`, `tmux-session`, `tmux-pane-id`, `shell-pid`, primary `worktree`, colon-joined worktree set, `plugin-root`, `agent-runtime`, `worker-runtime-args`, and intent. The driver renders the command through `scripts/orch-agent-runtime.sh launch-command`; it uses Claude startup pages/slash command only for Claude, while Codex receives execute-task in its initial prompt.
...
- On exit code 0:
  - Set master entry `state: in-progress`.
  - Read the new dispatch for the pane/worktree fields plus `agent-runtime`, `worker-runtime-args`, and **`reuse-agent-effective`** (`true | false | n/a`; legacy fallback `reuse-claude-effective`).
  - **Dispatch L2 with `intent=reuse-dispatch`**, passing the same runtime fields as first-dispatch plus `reuse-agent-effective`. Same-agent reuse sends the runtime-config block and runtime-appropriate execute-task instruction without launching; restart exits the old runtime then uses the adapter launch command; `n/a` uses a fresh first-dispatch flow.
