...
**Creating a worktree does not move the watchdog's anchor.** The `execute-task` watchdog locates the active task through `.zyz-worker/current-task` under the **session cwd** — and this skill deliberately does not `cd` into the new worktree, so the session cwd stays where it was. If a task runs inside a worktree created here, either keep the pointer under the session cwd with an **absolute** path to the task directory in it, or accept that resolution depends on the sibling-worktree fallback (which is a safety net, not a contract — with two concurrent runs in one repo it can attach to the wrong task). Getting this wrong makes the entire watchdog layer inert while looking perfectly healthy.
