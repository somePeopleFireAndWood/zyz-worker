...
**Creating a worktree does not move the watchdog's anchor.** The `execute-task` watchdog locates the active task through `.zyz-worker/current-task` under the **session cwd** — and this skill deliberately does not `cd` into the new worktree, so the session cwd stays where it was. If a task runs inside a worktree created here, keep the pointer under the session cwd with an **absolute** path to the task directory in it. There is no fallback: the resolver deliberately does not search sibling worktrees (an earlier fallback attached sessions to a concurrent session's task — issue #18), so a pointer the session cwd cannot see makes the entire watchdog layer inert while looking perfectly healthy. The §1 armed check is what catches that.
